...
export default function Register() {
  ...
  const validateField = async (name, value) => {
    ...
    switch (name) {
      case 'user_name':
        if (!value) {
          error = 'ชื่อผู้ใช้ห้ามเว้นว่าง';
        } else {
          const isDuplicate = await checkDuplicate('user_name', value);
          if (isDuplicate) error = 'ชื่อผู้ใช้นี้มีอยู่แล้ว';
        }
        break;
      case 'first_name':
      ...
      case 'email':
        if (!value) {
          error = 'อีเมลห้ามเว้นว่าง';
        } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
          error = 'รูปแบบอีเมลไม่ถูกต้อง';
        } else {
          const isDuplicate = await checkDuplicate('email', value);
          if (isDuplicate) error = 'อีเมลนี้มีอยู่แล้ว';
        }
        break;
      case 'password':
        if (!value) {
          error = 'รหัสผ่านห้ามเว้นว่าง';
  ...
  const checkDuplicate = async (field, value) => {
    try {
      const response = await fetch(`http://localhost:5000/check-duplicate?field=${field}&value=${value}`)
      const data = await response.json();
      return data.isDuplicate;
    } catch (error) {
      console.error('Error checking duplicate:', error);
      return false; // ในกรณีที่เกิดข้อผิดพลาด ไม่ควรบล็อกการใช้งาน
    }
  };
